...
def create_catalog():
    response = s.post("http://localhost:8080/api/catalogs", json={})
    return response.headers["Location"]


def create_offered_resource():
    response = s.post("http://localhost:8080/api/offers", json={})
    return response.headers["Location"]


def create_representation():
    response = s.post("http://localhost:8080/api/representations", json={})
    return response.headers["Location"]


def create_artifact():
    response = s.post(
        "http://localhost:8080/api/artifacts", json={"value": "Today is Monday"}
    )
    return response.headers["Location"]
...
response = contractRequest(
    "http://localhost:8080/api/ids/data", offers, artifact, False, obj
)
